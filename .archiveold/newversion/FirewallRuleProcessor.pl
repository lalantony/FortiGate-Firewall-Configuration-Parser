...
while( my $line = <> )
{
	chomp $line;
        # Start of Interface
	if( $line =~ /config\ssystem\sinterface/ )
	{
		my $intfid = '';
		my $mode_intf = 1;
		while( ($mode_intf == 1) && ($line = <>) )
		{
			if( $line =~ /^end/ )
			{
				$mode_intf = 0;
			...
			{
				my($f, $v) = &cleanvalueinterface($line);
				$interfaces{$intfid}->{$f} = $v;
			}
		}
	}
...
################################################################
# Spit out list of Interfaces
sub show_interfaces
{
	print "<h1>Interfaces</h1>\n";
	print "<table id=\"show_interfaces_table\" cellpadding=\"0\" cellspacing=\"0\" border=\"0\" class=\"display\">\n";
	print "<thead><tr><th>Interface</th><th>IP Address</th><th>VLAN ID</th></tr></thead>";
	foreach my $i (sort keys %interfaces)
	{
		my $rowcolour = $colour_r1;
		$rowcolour = $colour_r2 if( ($rowcount++ % 2) == 0 );
		my %a = %{ $interfaces{$i} };
                print "<tr><td>$i</td><td>$a{'ip'}</td><td>$a{'vlanid'}</td></tr>\n";    
	}
	print "</table>\n";
}
...
sub show_addresses
{
	...
	print "<thead><tr><th>Address</th><th>Value</th></tr></thead>";
	foreach my $i (sort keys %addr)
	{
		...
		if( $type eq '' )
		{
			...
			print "<tr bgcolor=\"$rowcolour\"><td>$i</td><td>$sn/$sm</td></tr>\n";
		}
...
sub show_pools
{
	...
	print "<thead><tr><th>Address Pool</th><th>Start Address</th><th>End Address(es)</th></tr></thead>";
...
sub show_policies
{
	...
	print "<thead><tr><th>Source Interface</th><th>Destination Interface</th><th>Source</th><th>Destination</th><th>Service</th><th>NAT</th><th>Action</th></tr></thead>";
	my $prev_pair = '';
	my $first_row = 1;
        my $xa = 0;
	foreach my $i (sort by_interface_order keys %rules)
	{
		...
		my $pair = "$r{'srcintf'} -> $r{'dstintf'}";
                ...
                print "<tr><td><b>$r{'srcintf'}</b></td><td><b>$r{'dstintf'}</b></td>";
		my $rowcolour = $colour_r1;
		$rowcolour = $colour_r2 if( ($rowcount++ % 2) == 0 );
